...
InfoSaniR_report <- function(data = "patho", Year = 2000, file = FALSE) {
    ...
    setwd(fileReport)
    knit2pdf("patho.Rnw", encoding = "UTF-8", quiet = F)
    knit2pdf("patho.Rnw", encoding = "UTF-8", quiet = T)
  
    #texi2pdf("patho.tex")
    #texi2dvi("patho.tex", pdf=T, clean = T)
} 
InfoSaniR_check <- function(data = "patho", Year = 2000, file = FALSE) {
  ## Checking inputs
  if (missing(data) && file == FALSE) {
    stop("data doit etre specifie car manquant")
  }
  ...
  if (exists(data, envir = .GlobalEnv) && file == FALSE) {
    setwd("~")
    if (is.element(paste("Verification", data, as.character(Year), 
      sep = "_"), list.files(getwd()))) {
      fileReport <- normalizePath(file.path(getwd(), 
        paste("Verification", data, as.character(Year), sep = "_")))
    } else {
      dir.create(file.path(getwd(), paste("Verification", data, 
        as.character(Year), sep = "_")))
      fileReport <- normalizePath(file.path(getwd(), 
        paste("Verification", data, as.character(Year), sep = "_")))
    }
    ...
    # Choosing a folder interactively
    fileExtract <- choose.dir(default = "", caption = "Le dossier donnees .RData InfoSani")
    fileReport <- choose.dir(default = "", caption = "Le dossier des verifications final")
  ...
  if (dim(Mistak0)[1]>1) {
    tab0 <- xtable((Mistak0),
      caption="Les erreurs de saisies repertoriées",label="tab0",digits=0)
    print(tab0, include.rownames = FALSE)
    write.xlsx(Mistak0, file = file.path(fileOutput,paste(paste('data_patho', year[1], sep='_'),'xlsx',sep='.')),
      row.names = F)
  }
  
  ####
  Mistak1 <- ddply(data0,.(Province, Region_Sanitaire, Departement, Etablissement, Nom, Maladies, Domaine, Annee,Mois),summarise,N=sum(na.omit(as.numeric(as.character(Effectif)))))
  #Fréquence des cas mensuelles de Maladiess
  
  Mistak2 <- ddply(Mistak1,.(Province, Region_Sanitaire, Departement, Etablissement, Nom, Maladies, Domaine, Annee),summarise,
    MAD=round(mad(na.omit(N)),0),
    MED=round(median(na.omit(N)),0)) 
  #Mediane absolue et mediane annuelle des cas de Maladiess
  
  Mistak3 <- merge(Mistak1,Mistak2)
  Mistake_Infectious <-subset(Mistak3,N-MED>5*MAD&N-MED>50&Domaine=='Infect')
  Mistake_NoInfectious <-subset(Mistak3,N-MED>5*MAD&N-MED>50&Domaine!='Infect')
  
  if (dim(Mistake_Infectious)[1]>1) {
    tab0 <- xtable((Mistake_Infectious),
      caption="Les erreurs de saisies repertoriées",label="tab0",digits=0)
    print(tab0, include.rownames = FALSE)}
