InfoSaniR_report <- function(data='patho',Year=2000,file=FALSE){
  ...
  if (exists(data, envir = .GlobalEnv)&&file==FALSE){
    setwd('~')
    if (is.element(paste('Report',data, as.character(Year), sep = '_'),
                    list.files(getwd()))){
        fileReport<- normalizePath(file.path(getwd(), paste('Report',data, 
                                   as.character(Year),sep='_')))}
    else {dir.create(file.path(getwd(), paste('Report',data, as.character(Year),sep='_')))
         fileReport<- normalizePath(file.path(getwd(), paste('Report',data, 
         as.character(Year),sep='_')))}
  }
  else {
    # Choosing a folder interactively
    fileExtract <- choose.dir(default = "", 
      caption = "Le dossier donnees .RData InfoSani")    
    fileReport <- choose.dir(default = "", 
      caption = "Le dossier du rapport final")
    
    ## Checking extracted data files
    ## specifier la recherche selon data mis en entree (patho,....All)
    FileData <- list.files(path = fileExtract, pattern ='.RData$')
    DirFileData <- as.list(dir(path = fileExtract, pattern ='.RData$',full.names=T))
